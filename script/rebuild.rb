...
class Rebuild < Thor
  ...
  def sitemaps
    ...
    File.open("#{Rails.root}/public/sitemaps/sitemap_index.xml", 'w') do |sitemap_index|
      ...
      sitemap_index.puts('<loc>https://www.extension.org/sitemaps/sitemap_communities.xml</loc>')
      sitemap_index.puts('</sitemap>')
      # pages
      if(index_pages_count == 1)
        sitemap_index.puts('<sitemap>')
        sitemap_index.puts('<loc>https://www.extension.org/sitemaps/sitemap_pages.xml</loc>')
        sitemap_index.puts('</sitemap>')
      else
        for i in (1..index_pages_count)
          sitemap_index.puts('<sitemap>')
          sitemap_index.puts("<loc>https://www.extension.org/sitemaps/sitemap_pages_#{i}.xml</loc>")
          sitemap_index.puts('</sitemap>')
        end
      end
    ...
    File.open("#{Rails.root}/public/sitemaps/sitemap_communities.xml", 'w') do |sitemap_communities|
      ...
      PublishingCommunity.launched.each do |community|
        community.tag_names.each do |name|
          ...
          sitemap_communities.puts("<loc>https://www.extension.org/#{URI.encode(Tag.url_display_name(name))}</loc>")
          sitemap_communities.puts('</url>')
        end
      end
    ...
    if(index_pages_count == 1)
      File.open("#{Rails.root}/public/sitemaps/sitemap_pages.xml", 'w') do |sitemap_pages|
        ...
        @pages.each do |page|
          ...
          sitemap_pages.puts("<loc>https://www.extension.org/pages/#{page.id}/#{page.url_title}</loc>")
