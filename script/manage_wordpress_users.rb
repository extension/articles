...
def update_from_darmok_users(connection,wordpressdatabase,mydatabase)
  ...
  sql +=  " FROM #{mydatabase}.users WHERE (NOT(#{mydatabase}.accounts.retired) AND (#{mydatabase}.accounts.vouched))"
...
sql = "REPLACE INTO #{wordpressdatabase}.wp_usermeta (umeta_id,user_id,meta_key,meta_value) SELECT #{mydatabase}.accounts.id, #{mydatabase}.accounts.id, 'wp_capabilities', '#{@admin_privs}' FROM #{mydatabase}.users WHERE #{mydatabase}.accounts.is_admin = 1;"
...
def set_expired_privs(connection,wordpressdatabase,mydatabase)
  ...
  sql = "REPLACE INTO #{wordpressdatabase}.wp_usermeta (umeta_id,user_id,meta_key,meta_value) SELECT #{mydatabase}.accounts.id, #{mydatabase}.accounts.id, 'wp_capabilities', '#{@expired_privs}' FROM #{mydatabase}.users WHERE #{mydatabase}.accounts.retired = 1;"
