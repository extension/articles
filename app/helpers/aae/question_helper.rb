...
def stringify_submitted_question_event(sq_event)
  ...
  when SubmittedQuestionEvent::RESOLVED 
    return "Resolved by <strong #{qw}>#{initiated_by_full_name}</strong> <span> #{humane_date(sq_event.created_at)} UTC</span>"
  when SubmittedQuestionEvent::NO_ANSWER
    return "No answer available was sent from <strong #{qw}>#{initiated_by_full_name}</strong><span> #{humane_date(sq_event.created_at)} UTC</span>"
  when SubmittedQuestionEvent::MARKED_SPAM
    return "Marked as spam by <strong #{qw}>#{initiated_by_full_name}</strong> <span> #{humane_date(sq_event.created_at)} UTC</span>"
  when SubmittedQuestionEvent::MARKED_NON_SPAM
    return "Marked as non-spam by <strong #{qw}>#{initiated_by_full_name}</strong> <span> #{humane_date(sq_event.created_at)} UTC</span>"
  when SubmittedQuestionEvent::REJECTED
    reject_msg = "Question Rejected by <strong #{qw}>#{initiated_by_full_name}</strong> <span> #{humane_date(sq_event.created_at)} UTC</span>"
  ...
  when SubmittedQuestionEvent::REOPEN
    return "Question reopened by <strong #{qw}>#{initiated_by_full_name}</strong> <span> #{humane_date(sq_event.created_at)} UTC</span>"
  when SubmittedQuestionEvent::CLOSED
    return "Question closed by <strong #{qw}>#{initiated_by_full_name}</strong> <span> #{humane_date(sq_event.created_at)} UTC</span>"
  when SubmittedQuestionEvent::PUBLIC_RESPONSE
    return "Comment posted by <strong #{qw}>public user</strong> <span> #{humane_date(sq_event.created_at)} UTC</span>"
  else
