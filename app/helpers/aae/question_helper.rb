...
def stringify_submitted_question_event(sq_event)
  ...
  when SubmittedQuestionEvent::ASSIGNED_TO 
    ...
    reassign_msg = "Assigned to <strong><a href='/ask/profile/#{sq_event.subject_user.login}'>#{sq_event.subject_user.fullname}</a></strong> by <strong>#{initiated_by_full_name}</strong> <span> #{humane_date(sq_event.created_at)}</span>"
