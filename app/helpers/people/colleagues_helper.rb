...
module People::ColleaguesHelper
  ...
  def makeusercsvstring(user)
    #eXtensionID,First Name,Last Name,Email,Title,Position,Institution,Affiliation,Location,County,Agreement Status,Account Created
  ...
  def colleague_community_connection_links(container,community,showuser,connectaction)
    remove_leader_link = colleague_community_connection_link(container,community,showuser,(community.is_institution? ? 'remove from inst. team' : 'remove leadership'),'removeleader',(community.is_institution? ? 'Remove from the Institutional Team' : 'Remove user from leadership'))
    ...
    add_leader_link = colleague_community_connection_link(container,community,showuser,(community.is_institution? ? 'add to inst. team' : 'make leader'),'addleader',(community.is_institution? ? 'Add to Institutional Team' : 'Make user a community leader'))
