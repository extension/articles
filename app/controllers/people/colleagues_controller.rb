...
class People::ColleaguesController < ApplicationController
  ...
  def tagcloud
    @tagcloud = User.notsystem_or_admin.validusers.tag_frequency(:order => 'name')
  ...
  def tags
    ...
    if(!params[:downloadreport].nil? and params[:downloadreport] == 'csv')
      reportusers = User.notsystem_or_admin.validusers.tagged_with_any(Tag.castlist_to_array(taglist),{:matchall => (match == 'matchall'),:order=> 'last_name,first_name'})
      csvfilename =  @page_title.tr(' ','_').gsub('\W','').downcase
      return csvuserlist(reportusers,csvfilename, @page_title)
    else
      @userlist =  User.notsystem_or_admin.validusers.tagged_with_any(Tag.castlist_to_array(taglist),{:matchall => (match == 'matchall'),:order=> 'last_name,first_name', :paginate => true, :page => params[:page]})
  ...
  def finduser
    if (!params[:searchterm].nil? and !params[:searchterm].empty?)
      ...
      if(admin_mode?)
        ...
        @userlist = User.notsystem_or_admin.validusers.patternsearch(params[:searchterm]).paginate({:order => 'last_name,first_name', :page => params[:page]})
      end
