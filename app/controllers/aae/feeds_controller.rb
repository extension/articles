...
class Aae::FeedsController < ApplicationController  
  ...
  def expertise
    ...
    @users = @category.get_experts(:select => "accounts.*, expertise_areas.created_at as added_at", 
  ...
  def escalation
    ...
    @alternate_link = url_for(linkoptions)  
    @submitted_questions = SubmittedQuestion.escalated(sincehours).filtered({:category => @filterparams.legacycategory}).listdisplayincludes.ordered('submitted_questions.last_opened_at desc')
    render_submitted_questions    
  end
