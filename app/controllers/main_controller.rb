...
class MainController < DataController
  ...
  def index
     ...
     @in_the_news = Article.categorized('news').all.ordered.limit(4)
     
     @community_highlights = Article.categorized('feature').ordered.limit(8)
     
     @latest_activities = Article.find(:all, :order => 'wiki_updated_at DESC', :limit => 4)
     @latest_faq = Faq.all.ordered.first

     @latest_article = @latest_activities[0]
     @second_latest = @latest_activities[1]
     
     date_conditions = ['start >= ? AND start < ?', @date, @date+5]
     @calendar_events = Event.all.find(:all, :conditions => date_conditions, :order => 'date ASC')

     @latest_learning_lesson = Article.categorized('learning lessons').ordered.first     
  end
  ...
  def category
    ...
    if @community
      ...
      @homage = Article.categorized(['homage', params[:category]]).ordered.first
      @in_this_section = Article.categorized(['contents', params[:category]]).ordered.first
      @community_highlights = Article.categorized(['feature', params[:category]]).
          ordered.limit(8)
    ...
    if @category.name == 'all'
      ...
      @news = Article.categorized('news').ordered.limit(3)
      @popular_learning_lessons = Article.categorized('learning lessons').ordered(Article.orderings['Most Useful']).limit(3)
      ...
      @news = Article.categorized(['news', params[:category]]).ordered.limit(3)
      @popular_learning_lessons = 
        Article.categorized(['learning lessons', params[:category]]).
          ordered(Article.orderings['Most Useful']).limit(3)
          
      @faqs = Faq.categorized(@category.name).ordered.limit(3)
      @calendar_events =  Event.categorized(@category.name).ordered.limit(5).after(@date)
    
      @articles = Article.categorized(params[:category]).
          ordered(Article.orderings['Most Useful']).limit(8) unless @community
        
      @recent_articles = Article.categorized(params[:category]).
