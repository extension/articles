...
class WikiImage < ActiveRecord::Base
  ...
  def self.make_hosted_images
    self.all.each do |wi|
      HostedImage.create(filename: wi.filename, path: wi.path, source_id: wi.page_id, source: 'copwiki', description: wi.description, copyright: wi.copyright)
    end
  end
