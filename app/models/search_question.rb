...
class SearchQuestion < ActiveRecord::Base
  ...
  named_scope :full_text_search, lambda{|options|
    ...
    {:select => "#{self.table_name}.*, MATCH(content,fulltitle) AGAINST ('#{match_string}') as match_score", :conditions => ["MATCH(content,fulltitle) AGAINST ('#{match_string}') AND status <> 'archived'"]}
  }
