...
module BounceEmail
  ...
  class Mail
    ...
    private
    def get_code(mail)
      return '9.9.7' if mail.subject.match(/delayed/i)
      return '9.9.8' if mail.subject.match(/(unzulässiger|unerlaubter) anhang/i)
      return '9.9.9' if mail.subject.match(/auto.*reply|vacation|vocation|(out|away).*office|on holiday|abwesenheits|autorespond|Automatische|eingangsbestätigung/i)
