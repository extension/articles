class ConvertInstitutionsToCommunities < ActiveRecord::Migration
  def self.up
    ...
    add_column :communities, "normalizedname", :string    
    ...
    execute "INSERT INTO communities (entrytype,memberfilter,name,created_at,updated_at,created_by,uri,normalizedname,location_id,public_uri,referer_domain,shared_logo,show_in_public_list,institution_code) SELECT 3,1,name,created_at,updated_at,1,uri,normalizedname,location_id,public_uri,referer_domain,shared_logo,show_in_public_list,code FROM old_institutions WHERE old_institutions.entrytype IN (1,3)"
    ...
    execute "INSERT INTO communityconnections (user_id,community_id,connectiontype,connected_by,created_at,updated_at) SELECT id,temp_institution_id,'member',1,created_at,NOW() from users WHERE temp_institution_id !=0 and temp_institution_id IS NOT NULL"  
