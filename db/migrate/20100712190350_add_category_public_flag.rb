class AddCategoryPublicFlag < ActiveRecord::Migration
  def self.up
    ...
    launched_community_tags = Tag.community_tags(retrieveoptions).map{|t| "'#{t.name}'"}
    categorylist = Category.find(:all, :conditions => "parent_id IS NULL and LOWER(name) IN (#{launched_community_tags.join(',')})", :order => 'name')
