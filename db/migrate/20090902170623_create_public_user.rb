class CreatePublicUser < ActiveRecord::Migration
  def self.up
    ...
    execute "INSERT IGNORE INTO public_users (email, first_name, last_name, valid, created_at, updated_at) SELECT LOWER(submitter_email), submitter_firstname, submitter_lastname, 1, NOW(), NOW() from submitted_questions WHERE submitter_email IS NOT NULL group by submitter_email"
